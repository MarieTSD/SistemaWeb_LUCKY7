...
if(!empty($_SESSION["shopping_cart"])) {
	foreach($_SESSION["shopping_cart"] as $key => $value) {
		if($_POST["descripcion"] == $key){
		unset($_SESSION["shopping_cart"][$key]);
		$status = '<script>swal("ELIMINADO", "PRODUCTO ELIMINADO CORRECTAMENTE", "success");</script>';
		}
		if(empty($_SESSION["shopping_cart"]))
...
if (isset($_POST['action']) && $_POST['action']=="change"){
  foreach($_SESSION["shopping_cart"] as &$value){
    if($value['descripcion'] === $_POST["descripcion"]){
			//$quantity = $_POST["quantity"];
		//if($quantity <=  $_SESSION["exist"]){
    ...
    //Eliminamos cantidad especifica de productos
    if (isset($_POST['action']) && $_POST['action']=="quitarProd"){
      foreach($_SESSION["shopping_cart"] as &$value){
        if($value['descripcion'] === $_POST["descripcion"]){
            $value['quantity'] = $value["quantity"] - $_POST["quantity"];//se hace la eliminacion
...
<div class="container">
	<table class="table">
		...
		?>
		<tr>
			<td><img class="pr" height="210px" src="data:image/jpg;base64,<?php echo base64_encode($product["image"]); ?>" /></td>
			<td><?php echo $product["name"]; ?><br />
			<form method='post' action=''>
			<input type='hidden' name='descripcion' value="<?php echo $product["descripcion"]; ?>" />
			<input type='hidden' name='action' value="remove" />
			<button type='submit' class='remove elim' >BORRAR</button>
			</form>
			</td>
			<td>
			<form method='post' action=''>
			...
			<!--Obtenemos la vantidad de productos que desea obtener-->
			<form action="this.form.submit()" method="post">
			  <input type="number" name="quantity" class='quantity' style="width:40px;" style="width:40px;">
			  <?php
                 if(($product["quantity"] >= $product["ex"])){
                     echo '<script>swal("Existencia limitada", "No contamos con los productos suficientes", "error");</script>';
                     $product["quantity"]=1;
                 }
               ?>
			</form>
			...
			<!--Mostramos cantidad solicitado-->
			<td><?php echo $product["quantity"]; ?></td>

			<!--Eliminamos cantidad especifica de producto-->
			<td>
			   <form method='post' action=''>
					<input type='hidden' name='descripcion' value="<?php echo $product["descripcion"]; ?>" />
					<input type='hidden' name='action' value="quitarProd" />
					<form action="this.form.submit()" method="post">
				  <input type="number" name="quantity" class='quantity' style="width:40px;">
				</form>
		...
		<tr>
			...
			<strong>TOTAL: <?php echo "$".$total_price; 
				$_SESSION['total2']= $total_price;?></strong>
			</td>
		</tr>
	</table>	
</div>	
  ...
  <?php
	...
	echo '<script>swal("CARRITO VACIO", "REGRESE PARA AGREGAR", "");</script>';
	}
?>
...
<div style="text-align:center; padding:10px;" class="hey1">
	<a id="hey" href="index.php" style="text-decoration: none; color:#28a745;"><button type='submit' class='buy btn btn-outline-success mt-2' id="hey" value="">REGRESAR A COMPRAR</button></a>
<a href="pais.php" style="text-decoration: none; color:#28a745;" class="hey"><button type='submit' class='buy btn btn-outline-success mt-2'>FINALIZAR COMPRA</button></a>
</div>
