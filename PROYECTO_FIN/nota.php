...
<html>
  <head>
    <meta charset="utf-8">
    <title>Ticket de la compra</title>
...
<!-- Codigo LOGIN-->  
   ...
   <!-- Hero section -->
    <section class="hero3 hero5">
     	<form action="#" method="POST" style="text-align: left; padding-left: 70px;">
...
<!--     <center>-->
        ...
        <p>Telefono: <?php echo $numtel; ?></p>
         <hr>

         <h3>PAGO</h3>

        <p>Subtotal de Compra: $<?php echo $_SESSION['total2']; ?></p>
                        <p>IVA: $<?php 
                                           if($_SESSION['pais'] == "Mexico" || $_SESSION['pais'] == "mexico" || $_SESSION['pais'] == "MEXICO")
                                            ...
                                            echo $res; 
                                       ?></p>
                        <p> Descuentos por cupones: $<?php 
                            echo $_SESSION['descuento']; 
                            ?></p>
                        <p>Gastos de Envio: $<?php 
                            echo $envio; 
                            ?></p>
                            
                        <p>TOTAL A PAGAR: $<?php 
                                       echo $SUMA = $_SESSION['total2'] + $res + $envio - $_SESSION['descuento']; 
                                        $_SESSION['suma'] = $SUMA; 
                                       ?></p>
                        <hr>
         ...
         <p>Modo de pago: <?php echo $_SESSION['pago']; ?></p>
          
<!--         <input type="button" name="enviar" value="Enviar Nota">-->
<!--        </center>-->
         </form>
         ...
         <?php
           include("sendemail2.php");//Mando a llamar la funcion que se encarga de enviar el correo electronico
				...
				$mail_username="lucky7original@gmail.com";//Nuestro correo
				$mail_userpassword="equipodinamita11";
				//$correo = $_POST['correo'];
				$mail_addAddress=$correo;//correo electronico que recibira el mensaje
				$template="email_template_nota.html";//Ruta de la plantilla HTML para enviar nuestro mensaje
				...
				$txt_sub= $_SESSION['total2']; //Subtotal de la cuenta
                $imp = $res; //impuestos
                $genv = $envio; //Gastos de envio
                $des = $_SESSION['descuento']; //descuento
                //$txt_sub = "123"; //subtotal
                $txt_tot = $_SESSION['suma']; //Total 
                $txt_pago = $_SESSION['pago']; //Modo de pago
				//$mail_subject=$nombre;
				
				sendemail2($imp,$genv,$des,$txt_sub,$txt_tot,$txt_pago,$mail_username,$mail_userpassword,$mail_setFromEmail,$mail_setFromName,$mail_addAddress,$template);//Enviar el mensaje
          ?>
    </section>
    <!-- End Hero section -->
